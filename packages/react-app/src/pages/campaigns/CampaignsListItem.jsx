...
const CampaignsListItem = (props) => {
  ...
  const [campaignTrigger, campaignResult, campaignLastPromiseInfo] =
    useLazyGetCampaignQuery();
  const [updateCampaign] = useUpdateCampaignMutation();
  const [destroyCampaign] = useDestroyCampaignMutation();
  const [toggleVerified] = useToggleVerifiedMutation();
  ...
  return (
    <tr key={props.react_unique_identifier_key}>
      ...
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div className="text-left">{props.allocation_currency}</div>
      </td>
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div className="form-switch">
          <input
            type="checkbox"
            id={`toggle-verified-${props.id}`}
            className="sr-only"
            checked={props.verified ?? false}
            onChange={async (e) => {
              toggleVerified({ id: props.id })
                .unwrap()
                .then((fulfilled) => {})
                .catch((reason) => {});
            }}
          ...
          <label
            ...
            htmlFor={`toggle-verified-${props.id}`}
          >
            ...
            children={
              <div className="border-t border-slate-200">
                <div className="pb-3 px-3 pt-3">
                  <span className={`font-bold`}>{props.title}</span> will be
                  deleted! Are you sure to delete this Campaign?
                  <div className="flex mt-5 mx-auto">
                    ...
                    <button
                      className="ml-auto mt-auto btn bg-red-500 hover:bg-red-600 text-white"
                      type="submit"
                      onClick={async (e) => {
                        e.stopPropagation();
                    ...
                    >
                      <span className="hidden xs:block ml-1">
                        Delete Campaign
                      </span>
                    </button>
                  </div>
