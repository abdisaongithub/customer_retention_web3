        ...
        <tr key={props.react_unique_identifier_key}>
                ...
                <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div className="form-switch">
                                    <input
                                        type="checkbox"
                                        id={`toggle-required-${props.id}`}
                                        className="sr-only"
                                        checked={props.required ?? false}
                                        onChange={async (e) => {
                                            toggleRequired({id: props.id})
                                            .unwrap()
                                            .then(fulfilled => {
                                            })
                                            .catch(reason => {
                                    ...
                                    />
                                    <label className="bg-slate-400" htmlFor={`toggle-required-${props.id}`}>
            ...
            <ModalBasic title={"Delete Task"} id={props.id} modalOpen={deleteModal} setModalOpen={(e) => {
                setDeleteModal(false)
            }} children={
                <div className="border-t border-slate-200">
                    <div className="pb-3 px-3 pt-3">
                        <span className={`font-bold`}>{props.label }</span> will be deleted! Are you sure to delete this
                        Task?
                        ...
                        <div className="flex mt-5 mx-auto">
                            ...
                            >
                                <span className="hidden xs:block ml-1">Delete Task</span>
                            </button>
                        </div>
                    </div>
                </div>
